...
interface SubjectClassesViewProps {
  ...
  classes: StudyClass[];
  onBack: () => void;
  onOpenAddClass: (subjectId: string) => void;
  onEditClass: (classItem: StudyClass) => void;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-lg">
        ...
        <button
          onClick={() => onOpenAddClass(subject.id)}
          className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm shadow-md shadow-indigo-600/30 transition-all cursor-pointer active:scale-95 shrink-0"
        >
          <Plus className="w-4 h-4" />
          <span>Add Class to {subject.name}</span>
        </button>
      </div>

      {/* Class Tiles Grid */}
      {subjectClasses.length === 0 ? (
        <div className="text-center py-16 px-4 rounded-3xl border-2 border-dashed border-slate-300 dark:border-slate-800 bg-white/40 dark:bg-slate-900/30 space-y-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">
              No classes added to {subject.name} yet
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
              Add your YouTube lecture link and Google Drive sheet to create the first class tile.
            </p>
          </div>
          <button
            onClick={() => onOpenAddClass(subject.id)}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-md transition-colors cursor-pointer"
          >
            <Plus className="w-4 h-4" />
                ...
                <div className="flex items-center justify-between gap-2 mb-3">
                  ...
                  >
                    <button
                      ...
                      title="Edit Class Links"
                    >
                      <Edit3 className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => {
                        if (window.confirm(`Delete class "${item.title}"?`)) {
